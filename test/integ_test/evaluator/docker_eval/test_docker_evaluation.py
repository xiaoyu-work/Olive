...
class TestDockerEvaluation:
    ...
    EVALUATION_TEST_CASE: ClassVar[List] = [
        ("PyTorchModel", get_pytorch_model, partial(get_accuracy_metric, "mnist_post_process"), 0.99),
        ("PyTorchModel", get_pytorch_model, get_latency_metric, 0.001),
        (
            "PyTorchModel",
            get_huggingface_model,
            partial(get_accuracy_metric, "mnist_post_process_hf", "prajjwal1_dataset"),
            0.1,
        ),
        ("PyTorchModel", get_huggingface_model, partial(get_latency_metric, "prajjwal1_dataset"), 0.001),
        ("ONNXModel", get_onnx_model, partial(get_accuracy_metric, "mnist_post_process"), 0.99),
        ("ONNXModel", get_onnx_model, get_latency_metric, 0.001),
        ("OpenVINOModel", get_openvino_model, partial(get_accuracy_metric, "mnist_post_process_openvino"), 0.99),
