...
def optimize(model_name: str, optimized_model_dir: Path):
    ...
    # protobuf 4.x aborts with OOM when optimizing mistral
    ...
    olive_config = None
    with (script_dir / "config_mistral.json").open() as fin:
    ...
    footprints_file_path = Path(__file__).resolve().parent / "footprints/mistral_gpu-dml_footprints.json"
    with footprints_file_path.open("r") as footprint_file:
        ...
        model_info = {
            "unoptimized": {
                "path": Path(next(unopimized_olive_model.get_model_components())[1].model_path).parent,
            },
            "optimized": {
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default="mistralai/Mistral-7B-Instruct-v0.1", type=str)
    parser.add_argument(
    ...
    config.hidden_size = model_to_hidden_size.get(args.model, 4096)
