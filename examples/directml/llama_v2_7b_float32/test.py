...
def run_onnx_llamav2(
    ...
    for idx in range(max_gen_len):
        ...
        x_1 = x
        attn_mask_1 = attn_mask
        k_cache_1 = k_cache[:, :, :pos]
        k_cache_2 = v_cache[:, :, :pos]
        pos1 = pos.astype(np.int64)

        # Decide the next token using your preferred sampling strategy.
        next_token = np.argmax(logits, axis=-1).astype(np.int64)
        output_tokens.extend(next_token)

        print(tokenizer.decode(torch.tensor(next_token).tolist()), end=" ", flush=True)
