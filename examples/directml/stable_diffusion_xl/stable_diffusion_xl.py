...
def run_inference_gui(pipeline, prompt, num_images, batch_size, image_size, num_inference_steps, base_images=None):
    ...
    scaled_image_size = min(image_size, 900)

    def image_completed(index, path):
        img = Image.open(path).resize((scaled_image_size, scaled_image_size))
    ...
    button_height = 30
    padding = 2
    window_width = image_cols * scaled_image_size + (image_cols + 1) * padding
    window_height = image_rows * scaled_image_size + (image_rows + 1) * padding + bar_height + button_height
    ...
    for row in range(image_rows):
        for col in range(image_cols):
            label = tk.Label(window, width=scaled_image_size, height=scaled_image_size, background="black")
            gui_images.append(label)
            label.place(x=col * scaled_image_size, y=row * scaled_image_size)

    y = image_rows * scaled_image_size + (image_rows + 1) * padding
