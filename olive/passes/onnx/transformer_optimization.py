...
class OrtTransformersOptimization(Pass):
    ...
    def _default_config(accelerator_spec: AcceleratorSpec) -> Dict[str, PassConfigParam]:
        ...
        config = {
            ...
            "num_key_value_heads": PassConfigParam(
                type_=int, default_value=None, description="Number of key/value attention heads."
            ),
    ...
    def _run_for_config(
        ...
        if config["float16"]:
            ...
            if config["use_gqa"]:
                ...
                if num_kv_heads is None:
                    raise ValueError(
                        "num_key_value_heads is not specified in the model attributes or the optimization config. "
                        "Please specify it in the model attributes or the optimization config."
                    )
    ...
    def _replace_mha_with_gqa(
        ...
        if replace_attn_mask_input_with_seq_len:
            ...
            for model_input in model.input:
            ...
            shape_node = onnx.helper.make_node(
                "Shape",
                inputs=["past_key_values.0.key"],
                outputs=["past_key_values.0.key_shape"],
                name=model.create_node_name("Shape"),
            )
            gather_node = onnx.helper.make_node(
                "Gather",
                inputs=["past_key_values.0.key_shape", "two"],
